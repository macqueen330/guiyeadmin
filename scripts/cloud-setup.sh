...
#   · 总时长控制在 5 分钟内，否则环境缓存建不起来 → 并行装、只装必需的
...
# 两个 CLI 并行装：容器里默认都没有。
# 装了不等于能用——还要环境的 Network access 放行对应域名，见 docs/cloud-container.md。
echo "→ 安装 vercel / supabase CLI"
npm i -g vercel@latest > /tmp/install-vercel.log 2>&1 &
PID_VERCEL=$!
npm i -g supabase@latest > /tmp/install-supabase.log 2>&1 &
PID_SUPABASE=$!
wait $PID_VERCEL || echo "  ⚠ vercel CLI 安装失败（见 /tmp/install-vercel.log），不阻塞会话"
wait $PID_SUPABASE || echo "  ⚠ supabase CLI 安装失败（见 /tmp/install-supabase.log），不阻塞会话"

echo "→ 版本"
node --version
command -v vercel   >/dev/null && vercel --version   || echo "  vercel: 未安装"
command -v supabase >/dev/null && supabase --version || echo "  supabase: 未安装"

# 出网探针。这里失败不该拦住会话启动，但要在启动日志里留下明确证据，
# 免得后面每个命令都撞 403 才发现是网络策略没配。
echo "→ 出网检查"
for host in api.github.com api.vercel.com api.supabase.com; do
